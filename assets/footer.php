	<footer>
		<article>
			<img src="<? echo $images ?>LogoReduzido.svg" id="LogoRodape" title="<? echo $empresa ?>" alt="<? echo $empresa ?>" />
			<p>
				<a href="<? echo $tellink ?>"><? echo $teltxt ?></a>
				<span class="dot">•</span>
				<a href="mailto:<? echo $email ?>"><? echo $email ?></a>
				<a href="<? echo $mapslocation ?>" target="_blank" class="breakDesktop"><? echo $endereco ?></a>
			</p>
		</article>
	</footer>

	<script type="text/javascript" src="<? echo $url ?>assets/javascript/jquery.js"></script>
	<script type="text/javascript" src="<? echo $url ?>assets/javascript/slick.js"></script>
	<script type="text/javascript" src="<? echo $url ?>assets/javascript/estrutura.js"></script>