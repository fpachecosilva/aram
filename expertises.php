<!DOCTYPE HTML>
<html>
	<head>
	<?php 
		include 'assets/config.php';
 
        $tituloPagina = 'Expertises - '. $empresa;
        $urlPagina = $Expertises;
	?>
<title><? echo $tituloPagina ?></title>
	<?php include 'assets/head.php'; ?>
	</head>
	<body class="Expertises">
        <?php include 'assets/header.php'; ?>
                
        <section class="PrimeiraSessao fullscreen-bg" style="background-image: url('<? echo $images ?>montanhas_1.jpg')";>            
            <article>
                
                    <h1>Um relacionamento se mantém no dia a dia. <br>Bons resultados também.</h1>
                    <p class="DesktopText"><strong>Somos uma gestora de patrimônios “boutique style”.</strong></p>
                    <p>
                        <span class="DesktopText">Temos um número de clientes mais exclusivo para poder ter muito tempo para as demandas de cada um deles.</span> É por isso que cada atendimento é personalizado e é desenvolvido de acordo com cada expectativa. É único.
                    </p>
                    <img src="<? echo $images ?>Seta.svg" />
                
            </article>
        </section>

        <section class="SegundaSessao">
            <h2>Como entendemos mais de você.</h2>
            <p>E alinhamos seus objetivos com nossas expertises.</p>
            <span class="Risco"></span>

            <div class="SliderWrapper">
                <p>Conheça o passo a passo</p>
                
                <div class="Slider">
                    
                    <div class="wrapper">
                        <div>01</div>

                        <div>
                            <span class="Bola"></span>                            
                        </div>    

                        <div class="SliderTexto">                            
                            Conversas e imersão<br> 
                            com o cliente.<br> 
                            Entendimento do modus<br> 
                            operandi da empresa.
                        </div>
                    </div>

                    <div class="wrapper">
                        <div>02</div>

                        <div>
                            <span class="Bola"></span>                            
                        </div>

                        <div class="SliderTexto">
                            Análise do cliente<br> 
                            e perfil de risco.
                        </div>
                    </div>
                    
                    <div class="wrapper">
                        <div>03</div>
                        
                        <div>
                            <span class="Bola"></span>
                        </div>

                        <div class="SliderTexto">
                            Alinhamento do<br> 
                            Mandato: planejamento<br> 
                            e estratégia de alocação
                        </div>
                    </div>

                    <div class="wrapper">
                        <div>04</div>

                        <div>
                            <span class="Bola"></span>
                        </div>

                        <div class="SliderTexto">
                            Implementação de<br> 
                            solução de<br> 
                            investimento customizada
                        </div>
                    </div>

                    <div class="wrapper">
                        <div>05</div>

                        <div>
                            <span class="Bola"></span>
                        </div>

                        <div class="SliderTexto">
                            Prestação de contas<br> 
                            Gerenciamento contínuo de risco<br> 
                            Calls e e-mails ao longo do mês<br> 
                            Report mensal
                        </div>
                    </div>

                </div>

                <div class="Detalhe">

                    <div class="bola-wrapper">
                        <span class="RiscoBola"></span>
                        <span class="Bola"></span>
                    </div>

                    <div></div>
                    <div></div>
                    <div></div>

                    <div class="bola-wrapper">
                        <span class="RiscoBola"></span>
                        <span class="Bola"></span>
                    </div>

                </div>

                <div class="Processo">
                    <img src="<? echo $images ?>Grafismo07.svg" />
                    <p>Ao longo de todo processo:</p>
                    <p>Revisões periódicas ao longo do processo, alinhadas ao objetivo profissional e pessoal do cliente.</p>
                </div>

            </div>
        </section>

        <section class="TerceiraSessao">
            <p class="Destaque">NOSSAS EXPERTISES</p>            

            <article>
            
                <div class="Boxes">

                    <div class="Box">
                        <p><strong>Gestão de<br> patrimônios no<br> Brasil e exterior</strong></p>
                        <p>01</p>
                    </div>

                    <div class="Box">
                        <p><strong>Assessoria<br> tributária,<br> sucessória e<br> proteção<br> patrimonial</strong></p>
                        <p>02</p>

                    </div>

                    <div class="Box">
                        <p><strong>Assessoria de<br> planejamento<br> corporativo e real<br> estate:</strong></p>
                        <p>network e guidance<br> para tomada de ação<br> no mercado</p>
                        <p>03</p>
                    </div>

                    <div class="Box">
                        <p><strong>Cliente<br> Experience: </strong></p>
                        <p>hub de conexões,<br> eventos de incentivo<br> à arte, cultura e<br> filantropia</p>
                        <p>04</p>
                    </div>

                    <div class="Box">
                        <p><strong>Controle, execução e suporte operacional:</strong></p>
                        <p>controle mensal dos relatórios das instituições<br> e das movimentações,<br> abertura de contas,<br> movimentação e<br> realocações</p>
                        <p>05</p>
                    </div>

                    <div class="Box">
                        <p><strong>Investimentos alternativos:</strong></p>
                        <p>ilíquidos, ventures e real estate, entre outros</p>
                        <p>06</p>
                    </div>

                    <div class="Box">
                        <p><strong>Investimentos<br> de impacto: </strong></p>
                        <p>Fazemos e incentivamos,<br> gerando resultados com<br> responsabilidade</p>
                        <p>07</p>
                    </div>

                    <div class="Box">
                        <p><strong>Gerenciamento de risco:</strong></p>
                        <p>Análise constante e<br> dinâmica do mercado,<br> alinhado com o perfil e<br> objetivos do cliente</p>
                        <p>08</p>
                    </div>
                </div>

            </article>
        
            <div class="Praticas">
                <p class="Destaque">NOSSAS PRÁTICAS</p>
                
                <div class="Wrapper">

                    <div class="esq">
                        <h2>Compliance</h2>
                        <p>
                            Acima da proximidade de nosso atendimento está a segurança de nossos clientes.<br><br>

                            Da conduta de nossa equipe e práticas internas a normas federais, internacionais e dos órgãos reguladores do mercado, está em nosso âmago proteger seus dados, recursos 
                            e investimentos.
                        </p>
                    </div>

                    <div>
                        <img src="<? echo $images ?>Grafismo08.svg" />
                    </div>

                    <div class="dir">
                        <h2>Flexibilidade</h2>
                        <p>
                            Entendemos de números. E ainda mais de pessoas.<br><br>

                            Escolhemos ter um portfólio reduzido para poder estar sempre perto, dando abertura para nossos clientes movimentarem seus capitais de maneira rápida e alinhados com seus novos objetivos de vida.
                        </p>
                    </div>

                </div>
            </div>
        </section>

        <section class="QuartaSessao">
            <div class="DesktopItem"></div>
            <div class="MobileItem"></div>
            <a href="<? echo $Sobre ?>">Sobre nós ></a>
        </section>
        

        <?php include 'assets/footer.php'; ?>

    <script type="text/javascript">
        $('.LinkMenu:eq(0)').addClass('SessaoAtual');
    </script>
    </body>
</html>